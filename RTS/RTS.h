	...
	public:
		...
		int loadinput(char *filename) {
			...
			string cmd = "sed \'s/\t/ /g\'" + string(filename) + " > sample.clean";
			 // original "sed \'s/\t/ /g\'", replaces tabs with spaces
			//possible alt for the sed string is tr '\t' ' ' or 'y/\t/ /' found at http://stackoverflow.com/questions/12920121/sed-program-what-does-it-do
			//tr seems to take user input ad infintum until killed with ctrl+d
			system(cmd.c_str());
		...
		int run_sched(){
			...
			while(!doneWithLast) {
				cout << "prepare to check if goto skip" << endl;
